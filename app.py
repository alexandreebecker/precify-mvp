# ==============================================================================
# Precify.AI - SPRINT 2.5 - Dashboard Interativo e Tema (Versão Completa)
# ==============================================================================
...
def render_dashboard():
    """Renderiza o novo painel principal com cards interativos."""
    ...
    for i, categoria in enumerate(categorias):
        with cols[i]:
            with st.container(border=True):
                ...
                st.markdown("---")
                if st.button(f"Iniciar", key=f"start_{categoria.lower().replace(' ', '_')}", use_container_width=True):
...
def render_form_campanha_online():
    with st.form(key="briefing_online_form"):
        ...
        dados_form = {"tipo_campanha": "Campanha Online", 'briefing_semantico': st.text_area("Descreva o objetivo principal da campanha", help="Ex: 'Queremos uma campanha para aumentar o alcance...'")}
        st.divider()
        col1, col2 = st.columns(2)
        with col1:
            def render_form_campanha_360():
    with st.form(key="briefing_360_form"):
        st.info("Detalhe a campanha integrada."); dados_form = {"tipo_campanha": "Campanha 360"}; dados_form['briefing_semantico'] = st.text_area("Descreva o objetivo"); st.divider(); col1, col2 = st.columns(2)
        with col1:
        ...
        with col2:
            ...
            duracao = st.date_input("Duração", (date.today(), date.today() + timedelta(days=90)));
...
def render_form_projeto_estrategico():
    with st.form(key="briefing_estrategico_form"):
        st.info("Descreva o desafio de negócio."); dados_form = {"tipo_campanha": "Projeto Estratégico"}; dados_form['desafio_principal'] = st.text_area("Qual o desafio?"); st.divider(); col1, col2 = st.columns(2)
        with col1:
            if st.radio("Ações anteriores?", ("Não", "Sim")) == "Sim": dados_form['detalhes_acoes_anteriores'] = st.text_area("Descreva")
            dados_form['apodados_form['canais'] = st.multiselect("Canais digitais", ["Instagram", "TikTok", "YouTube", "Facebook", "LinkedIn", "Google Ads", "Outro"])
        ...
        if st.form_submit_button("Analisar Briefing ➡️"): st.session_state.dados_briefing = dados_form; st.session_state.orcamento_step = 3; st.rerun()
...
def render_form_campanha_offline():
    with st.form(key="briefing_offline_form"):
        ...
        dados_form = {"tipo_campanha": "Campanha Offline", 'briefing_semantico': st.text_area("Descreva o objetivo principal", help="Ex: 'Participar da feira XYZ...'")}
        st.divider()
        col1, col2 = st.columns(io_estrategico_criativo'] = (st.radio("Apoio criativo?", ("Não", "Sim")) == "Sim"); dados_form['diagnostico_reputacao'] = (st.radio("Diagnóstico?", ("Não", "Sim")) == "Sim")
        with col2:
            dados_form['imprensa_influenciadores'] = st.multiselect("Imprensa/Influencers?", ["Assessoria", "Mkt Influência"])
            if st.radio("Verba disponível?", ("Não", "Sim")) == "Sim": dados_form['valor_verba'] = st.number_input("Valor (R$)", 0.0, format="%.2f")
        if st.form_submit_button("Avançar para Escopo ➡️"):
            st.session_state.dados_briefing = dados_form; st.session_state.orcamento_step = 3; st.rerun()

def calcular_orcamento(entregaveis, configs):
    custo_total_equipe = sum(aloc['horas'] * aloc['custo_hora'] for entr in entregaveis for aloc in entr.2)
        with col1:
        ...
        if st.form_submit_button("Analisarget('alocacoes', [])); taxa_coord_p = configs.get('taxa_coordenacao', 0)/100; custos_fixos_p = configs.get('custos_fixos', 0)/100; margem_lucro_p = configs.get('margem_lucro', 0)/100; impostos_p = configs.get('impostos', 0)/100
    custo_c_coord = custo_total_equipe * (1 + taxa_coord_p); custo_c_fixos = custo_c_coord * (1 + custos_fixos_p); lucro = custo_c_fixos * margem_lucro_p; sub_impostos = Briefing ➡️"): st.session_state.dados_briefing = dados_form; st.session_state.orcamento_step = 3; st.rerun()

def render_form_campanha_360():
    with st.form(key="briefing_360_form"):
        st.info("Detalhe a campanha integrada.")
        dados_form = {"tipo_campanha": "Campanha 360", 'briefing_semantico': st.text_area("Descreva o objetivo", help="Ex: 'Lançar nova identidade visual...'")}
        st.divider()
        col1, col2 = st.columns(2)
        with col1:
            dados_form['canais_envolvidos'] = st.multiselect("Canais", ["Digital", "Físico (Evento)", "Mídia custo_c_fixos + lucro; impostos = sub_impostos * impostos_p; total = sub_impostos + impostos
    return {"custo_total_equipe": custo_total_equipe, "valor_taxa_coordenacao": custo_c_coord - custo_total_equipe, "valor_custos_fixos": custo_c_fixos - custo_c_coord, "subtotal (TV, Rádio)"])
            if st.radio("Verba por frente?", ("Não", "Sim"), horizontal=True) == "Sim": dados_form['verba_detalhada'] = st.text_area("_antes_lucro": custo_c_fixos, "valor_lucro": lucro, "subtotal_antes_Detalhar verbas")
        with col2:
            dados_form['acompanhamento_estrategico'] = (st.radio("Acompanhamento?", ("Não", "Sim"), horizontal=True) == "Simimpostos": sub_impostos, "valor_impostos": impostos, "valor_total_cliente": total}

# --- 3. FUNÇÕES DE DADOS (FIRESTORE) E AUTH ---
@st.cache_")
            duracao = st.date_input("Duração", value=(date.today(), date.today() + timedelta(days=90)))
            if len(duracao) == 2: dados_formresource
def initialize_firebase():
    try: creds_dict = json.loads(st.secrets["FIREBASE_SECRET_COMPACT_JSON"]);
    except: creds_dict = {} # Fallback para['duracao_inicio'], dados_form['duracao_fim'] = str(duracao[0]), str(duracao[1])
            dados_form['segmentacao_geografica'] = st.selectbox("Segment desenvolvimento local se o segredo não estiver definido
    if not firebase_admin._apps: firebase_admin.initializeação", ["Local", "Regional", "Nacional"])
        if st.form_submit_button("An_app(credentials.Certificate(creds_dict))
    return firestore.client()

@st.alisar Briefing ➡️"): st.session_state.dados_briefing = dados_form; st.session_state.orcamento_step = 3; st.rerun()

def render_form_projetocache_data(ttl=300)
def carregar_perfis_equipe(_db, agencia_estrategico():
    with st.form(key="briefing_estrategico_form"):
_id):
    try: return [{"id": doc.id, **doc.to_dict()} for doc in _db.collection('agencias').document(agencia_id).collection('perfis_equipe').        st.info("Descreva o desafio de negócio ou de marca.")
        dados_form = {"tipo_campanha": "Projeto Estratégico", 'desafio_principal': st.text_area("Qual o desafio?", helpstream()]
    except Exception as e: st.error(f"Erro perfis: {e}"); return []

@="Ex: 'Reverter percepção...'")}
        st.divider()
        col1, col2 = stst.cache_data(ttl=300)
def carregar_configuracoes_financeiras(_db, agencia_id):
    try: doc = _db.collection('agencias').document(agencia_id)..columns(2)
        with col1:
            if st.radio("Ações anteriores?", ("Não", "Sim")) == "Sim": dados_form['detalhes_acoes_anteriores'] = st.get(); return doc.to_dict().get('configuracoes_financeiras', {}) if doc.exists elsetext_area("Descreva")
            dados_form['apoio_estrategico_criativo'] = {}
    except Exception as e: st.error(f"Erro configs: {e}"); return {}

@ (st.radio("Apoio criativo?", ("Não", "Sim")) == "Sim")
            dados_form['diagnostico_reputacao'] = (st.radio("Diagnóstico?", ("Não", "st.cache_data(ttl=300)
def carregar_orcamentos(_db, agencia_Sim")) == "Sim")
        with col2:
            dados_form['imprensa_influenciadoresid):
    try: orc_ref = _db.collection('orçamentos').where('agencia_id', '==', agencia_id).order_by('data_orcamento', direction=firestore.Query.DESCENDING'] = st.multiselect("Imprensa/Influencers?", ["Assessoria", "Mkt Influência"])).stream(); return [doc.to_dict() for doc in orc_ref]
    except Exception as
            if st.radio("Verba disponível?", ("Não", "Sim")) == "Sim": dados_form['valor_verba'] = st.number_input("Valor (R$)", min_value=0. e: return []

def salvar_orcamento_firestore(_db, agencia_id, user, dados):
    try: _db.collection('orçamentos').add({"agencia_id": agencia_id, "0, format="%.2f")
        if st.form_submit_button("Analisar Briefinguid_criador": user['uid'], "email_criador": user['email'], "data_orcamento ➡️"): st.session_state.dados_briefing = dados_form; st.session_state.orcamento_step = 3; st.rerun()

def calcular_orcamento(entregaveis": firestore.SERVER_TIMESTAMP, **dados}); st.cache_data.clear(); return True
    except Exception as e: st.error(f"Falha ao salvar: {e}"); return False

def sign_up(, configs):
    custo_total_equipe = sum(aloc['horas'] * aloc['email, password, name):
    try: user = auth.create_user(email=email, password=custo_hora'] for entr in entregaveis for aloc in entr.get('alocacoes', []))
    taxa_coord_p = configs.get('taxa_coordenacao', 0)/password, display_name=name); db.collection('agencias').document(user.uid).set({'nome': f"Agência de {name}"}); st.success("Registrado!")
    except Exception as e:100; custos_fixos_p = configs.get('custos_fixos', 0)/100
    margem_lucro_p = configs.get('margem_lucro',  st.error(f"Erro registro: {e}")

# --- 4. INICIALIZAÇÃO E LÓGICA DE LOGIN ---
db = initialize_firebase()
if 'logged_in' not in st.session_state: st.session_state.logged_in = False

if not st.session_state.logged_0)/100; impostos_p = configs.get('impostos', 0)/100
    custo_c_coord = custo_total_equipe * (1 + taxa_coord_p); custo_c_fixos = custo_c_coord * (1 + custos_fixos_p)in:
    st.title("Bem-vindo ao Precify.AI"); choice = st.selectbox("Acessar", ["Login", "Registrar"], label_visibility="collapsed")
    if choice == "Login":
        with st.form("login_form"):
            email=st.text_input("Email"); password
    ...
    return {"custo_total_equipe": custo_total=st.text_input("Senha",type="password")
            if st.form_submit_button("Login"):
                try: user=auth.get_user_by_email(email); st.session_state.logged_equipe, "valor_taxa_coordenacao": custo_c_coord - custo_total_equipe, "valor_custos_fixos": custo_c_fixos - custo_c_coord, "subtotal_antes_lucro": custo_c_fixos, "valor_lucro": lucro_in=True; st.session_state.user_info={"name":user.display_name,"email":user.email,"uid":user.uid}; st.rerun()
                except Exception: st.error("Credenciais inválidas.")
    else:
        with st.form("register_form"):
            name=st., "subtotal_antes_impostos": sub_impostos, "valor_impostos": impostos, "valor_total_cliente": total}

# --- 3. FUNÇÕES DE DADOS (FIRESTORE) E AUTH ---
@st.cache_resource
def initialize_firebase():
    try:
        creds_text_input("Nome"); email=st.text_input("Email"); password=st.text_input("Senha",type="password")
            if st.form_submit_button("Registrar"): sign_up(email, password, name)
else:
    user_info=st.session_state.user_infodict = json.loads(st.secrets["FIREBASE_SECRET_COMPACT_JSON"])
        if not firebase_admin._apps: firebase_admin.initialize_app(credentials.Certificate(creds_dict))
        return firestore.client()
    except Exception as e: st.error(f"FALHA FIRE; agencia_id=user_info['uid']; nome=user_info.get('name')
    st.sidebar.title(f"Olá, {nome.split()[0]}!" if nome and nome.strip() else "Olá!")
    if st.sidebar.button("Logout"):
        for k in list(st.BASE: {e}"); return None

@st.cache_data(ttl=300)
def carregar_perfis_equipe(_db, agencia_id):
    try: return [{"id": doc.id, **doc.to_dict()} for doc in _db.collection('agencias').document(agenciasession_state.keys()): del st.session_state[k]
        st.rerun()
    st.sidebar.divider()
    
    view_options = ["Painel Principal", "Novo Orçamento", "Meus Orçamentos", "Configurações"]
    if 'current_view' not in st.session_state_id).collection('perfis_equipe').stream()]
    except Exception as e: st.error(f"Erro perfis: {e}"); return []

@st.cache_data(ttl=300)
def carregar_configuracoes_financeiras(_db, agencia_id):
    try:
 or st.session_state.current_view not in view_options:
        st.session_state.current_view = "Painel Principal"
    
    if st.session_state.get('redirect_to_orcamentos', False):
        st.session_state.current_view = "Meus Orçamentos        doc = _db.collection('agencias').document(agencia_id).get()
...
@st.cache_"
    ...
    view = st.sidebar.radio("Menu", view_options, index=current_index)
    
    if view != st.session_state.current_viewdata(ttl=300)
...
def salvar_orcamento_firestore(_db, agencia_id, user, dados):
    try:
        _db.collection('orçamentos').add({"agencia_id": agencia_id,el Principal":
        render_dashboard()
    
    elif view == "Meus Orçamentos":
        st.header("Histórico de Orçamentos")
        orcamentos_data = carregar_orcamentos(db, agencia_id)
        if not orcamentos_data: st.info("Nenhum orçamento salvo ainda "uid_criador": user['uid'], "email_criador": user['email'], "data_orcamento": firestore.SERVER_TIMESTAMP, **dados})
        st.cache_data.clear()
        return True
    except Exception as e: st.error(f"Falha ao salvar: {e}"); return False
...
def.")
            ...
            for orc_data in orcamentos_data:
                ...
                data_formatada = data_orc.strftime("%d/%m/%Y") if hasattr sign_up(email, password, name):
    try:
...
# --- 4. IN(data_orc, 'strftime') else "Sem data"
                with st.expander(f"**{orc_data.get('nome_cliente', 'N/A')}** | {data_formatada} | **R$ {res.get('valor_total_cliente', 0):.2f}**"):ICIALIZAÇÃO E LÓGICA DE LOGIN ---
db = initialize_firebase()
...
if not st.session_state.logged_in:
    st.title
                    st.subheader("Detalhes do Orçamento"); st.markdown(f"**Custo Equipe:** `R$ {res.get('custo_total_equipe', 0):.2f}` | **Tx. Coordenação:** `R$ {res.get('valor_taxa_coordenacao', 0):("Bem-vindo ao Precify.AI"); choice = st.selectbox("Acessar", ["Login", "Registrar"], label_visibility="collapsed")
    if choice == "Login":
        with st.form("login_form"):
            email=st.text_input("Email"); password=st.text_input("Senha",type.2f}` | **Custos Fixos:** `R$ {res.get('valor_custos_fixos', 0):.2f}` | **Lucro:** `R$ {res.get('valor_lucro', 0):.2f}` | **Impostos:** `R$ {res.get('valor_im="password")
            if st.form_submit_button("Login"):
                try: user=auth.get_user_by_email(email); st.session_state.logged_in=True; st.session_state.user_info={"name":user.display_name,"email":user.email,"uid":postos', 0):.2f}`")
                    st.subheader("Escopo Final"); [st.write(f"- {item['descricao']}") for item in orc_data.get('escopo_final', [])]
                    st.subheader("Dados do Briefing"); briefing_dict = orc_data.get('briefing', {}); [st.markdown(f"**{k.replace('_', ' ').title()}:** {vuser.uid}; st.rerun()
                except Exception: st.error("Credenciais inválidas.")
    else:
        with st.form("register_form"):
        ...
        if st.session_state.orcamento            if st.form_submit_button("Registrar"): sign_up(email, password, name)
else:
    user_info=st.session_state.user_info; agencia_id=user_info['uid']; nome=user_info.get('name')
    st.sidebar.title(f"_step == 2:
            ...
            elif cat=="CampanhaOlá, {nome.split()[0]}!" if nome and nome.strip() else "Olá!")
    if st.sidebar.button("Logout"):
        for k in list(st.session_state.keys()): del st.session_state[k]
        st.rerun()
    st.sidebar.divider()
    
     Offline": render_form_campanha_offline()
            elif cat=="Campanha 360": render_form_campanha_360()
            elif cat=="Projeto Estratégico": render_form_projeto_estrategico()
            if st.button("⬅️ Voltar ao Painel"): st.session_view_options = ["Painel Principal", "Novo Orçamento", "Meus Orçamentos", "Configurações"]
    if 'current_view' not in st.session_state or st.session_state.current_view not in view_options:
        st.session_state.current_view = "Painel Principal"state.current_view = "Painel Principal"; st.rerun()
    ...
    current_index = view_options.index(st.session_state.current_view not in st.session_state: st.session_state.entregaveis = [{"descricao": item} for item in get_sugestoes_entregaveis(cat)]
    ...
    if view != st.session_state.current_view:
        st.session_state.current_view = view
        if view != "Novo Orçamento":
            for k in2.button("Adicionar",use_container_width=True) and novo: st.session_state.entregaveis.append({"descricao":novo}); st.rerun()
            for i, item in enumerate(st.session_state.entregaveis):
                c1,c2=st.columns([4,1]); c [k for k in st.session_state if k.startswith('orcamento_')]: del st.session_state[k]
        st.rerun()
    ...
    if view == "Painel Principal":
        ...
        st.header("Histórico de Orç1.write(f"• {item['descricao']}")
                if c2.button("Remover", key=f"rm_{i}",use_container_width=True): st.session_state.entregaveis.pop(i); st.rerun()
            st.divider(); c1,c2=stamentos")
        orcamentos_data = carregar_orcamentos(db, agencia_id)
        if not orcamentos_data: st.info("Nenhum orçamento salvo ainda. Crie seu primeiro no Painel Principal.")
        else:
            for orc_data in orcamentos_data:
                res = orc.columns(2)
            if c1.button("⬅️ Editar Briefing"): st.session_state.orcamento_step=2; st.rerun()
            if c2.button("Avançar para Alocação ➡️",type="primary",use_container_width=True,disabled=not st.session_state._data.get('resultado_financeiro', {}); data_orc = orc_data.get('data_orcamento')
                data_formatada = data_orc.strftime("%d/%m/%Y") if hasattr(data_orc, 'strftime') else "Sem data"
                with st.expander(f"**entregaveis): st.session_state.orcamento_step=4; st.rerun()

        elif st.session_state.orcamento_step == 4:
            st.header("👨‍💻 Alocação de Horas"); perfis = carregar_perfis_equipe(db, agencia_{orc_data.get('nome_cliente', 'N/A')}** | {data_formatada} | **R$ {res.get('valor_total_cliente', 0):.2f}**"):id)
            if not perfis: st.warning("Cadastre perfis em 'Configurações'."); st.
                    st.subheader("Detalhes do Orçamento")
                    st.markdown(f"**Custo Equipe:** `R$ {res.get('custo_total_equipe', 0):.2fstop()
            nomes_perfis = [p['funcao'] for p in perfis]; total_h = 0
            for i, entr in enumerate(st.session_state.entregaveis):
}` | **Tx. Coordenação:** `R$ {res.get('valor_taxa_coordenacao', 0):.2f}` | **Custos Fixos:** `R$ {res.get('valor_custos_fixos', 0):.2f}` | **Lucro:** `R$ {res                if 'alocacoes' not in entr: entr['alocacoes']=[]
                ...
                with st.expander(f"**{i+1}. {entr['descricao']}** ({horas_e}h)"):
                    for j, aloc in enumerate(entr['alocacoes']):
                        c1,c2.get('valor_lucro', 0):.2f}` | **Impostos:** `R$ {res.get('valor_impostos', 0):.2f}`")
                    st.subheader("Escopo Final"); [st.write(f"- {item['descricao']}") for item in orc_data.get('escopo_final', [])]
                    st.subheader("Dados do Briefing"); st.json(orc,c3=st.columns([2,1,1]); c1.write(f"• {aloc['perfil_funcao']}"); c2.write(f"{aloc['horas']}h")
_data.get('briefing', {}))
    
    elif view == "Novo Orçamento":
        if 'orcamento_step' not in st.session_state: st.session_state.current_view = "Painel Principal"; st.rerun()
        
        if st.session_state.orcamento_step ==                        if c3.button("X",key=f"rem_{i}_{j}"): st.session_state.entregaveis[i]['alocacoes'].pop(j); st.rerun()
                    st.divider(); c1,c2,c3=st.columns([2,1,1]); sel=c1.selectbox("Perfil",nomes_perfis,key=f"sel_{i}",index=None); h=c2.number_input("Horas",0.5,step=0.5,key=f 2:
            st.header(f"Briefing: {st.session_state.get('orcamento_categoria')}"); cat = st.session_state.get('orcamento_categoria');
            if cat=="Campanha Online": render_form_campanha_online()
            elif cat=="Campanha Offline": render_form_campanha_offline()
            elif cat=="Campanha 360": render_form_camp"h_{i}")
                    if c3.button("Adicionar",key=f"add_{i}",disabled=not sel):
                        p_data = next((p for p in perfis if p['funcao']==sel),None)
                        if p_data: st.session_state.entregaveis[i]['alocacoes'].append({"perfil_id":p_data['id'],"perfil_funcao":anha_360()
            elif cat=="Projeto Estratégico": render_form_projeto_estrategico()
            if st.button("⬅️ Voltar ao Painel"): st.session_state.current_view = 'Painel Principal'; st.rerun()

        elif st.session_state.orcamento_step == p_data['funcao'],"custo_hora":p_data['custo_hora'],"horas":h}); st.rerun()
            st.divider(); st.header(f"Total Horas: {total_h}h")
            c1,c2=st.columns(2)
            if c3:
            st.header("🤖 Validação do Escopo"); cat = st.session_state.get('orcamento_categoria', 'N/A')
            if 'entregaveis' not in st.session_state: st.session_state.entregaveis = [{"descricao": item} for item in get1.button("⬅️ Editar Escopo"): st.session_state.orcamento_step=3; st.rerun()
            if c2.button("Calcular Orçamento ➡️",type="primary",use_container_width=True,disabled=(total_h==0)): st.session_state.orcamento__sugestoes_entregaveis(cat)]
            c1,c2=st.columns([3,1]); novo=c1.text_input("Novo",label_visibility="collapsed")
            if c2.button("Adicionar",use_container_width=True) and novo: st.session_state.step=5; st.rerun()
            ...
            if not configs: st.warning("Configs financeiras não encontradas.")entregaveis.append({"descricao":novo}); st.rerun()
            for i, item in enumerate(st.session_state.entregaveis):
                c1,c2=st.columns([4
            ...
            with st.expander("Ver detalhamento"): st.markdown(f"**Custo Equipe:** `R,1]); c1.write(f"• {item['descricao']}")
                if c2.button("Remover", key=f"rm_{i}",use_container_width=True): st.session_state.entregaveis.pop(i); st.rerun()
            st.divider(); c1,$ {resultado['custo_total_equipe']:.2f}`\n\n**+ Tx. Coordenação ({configs.get('taxa_coordenacao',0)}%):** `R$ {resultado['valor_taxa_coordenacao']:.2f}`\n\n**+ Custos Fixos ({configs.getc2=st.columns(2)
            if c1.button("⬅️ Editar Briefing"): st.session_state.orcamento_step=2; del st.session_state.entregaveis; st.rerun()
            if c2.button("Confirmar Escopo ➡️",type="primary",use_container_width=True,disabled=not st.session_state.entregaveis): st.('custos_fixos',0)}%):** `R$ {resultado['valor_custos_fixos']:.2f}`\n\n**= Subtotal Op.:** `R$ {resultado['subtotal_antes_lucro']:.2f}`\n\n**+ Lucro ({configs.get('margem_lucro',0)}%):** `R$ {resultado['valor_lucro']:.2session_state.orcamento_step=4; st.rerun()
            ...
            if c2.button("Salvar Ornomes_perfis = [p['funcao'] for p in perfis]; total_h = 0
            for i, entr in enumerate(st.session_state.entregaveis):
                ...
                with st.expander(f"**{i+1}. {entr['descricao']}** ({horas_e}h)"):
                    ...
                    st.çamento ✅",type="primary",use_container_width=True,disabled=not nome_cliente):
