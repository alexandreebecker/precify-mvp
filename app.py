        ...
        if not orcamentos_data: st.info("Nenhum orçamento salvo ainda.")
            ...
            for orc_data in orcamentos_data:
                ...
                with st.expander(f"**{orc_data.get('nome_cliente', 'N/A')}** | {data_formatada} | **R$ {res.get('valor_total_cliente', 0):.2f}**"):
                    st.subheader("Detalhes do Orçamento"); st.markdown(f"**Custo Equipe:** `R$ {res.get('custo_total_equipe', 0):.2f}` | **Tx. Coordenação:** `R$ {res.get('valor_taxa_coordenacao', 0):.2f}` | **Custos Fixos:** `R$ {res.get('valor_custos_fixos', 0):.2f}` | **Lucro:** `R$ {res.get('valor_lucro', 0):.2f}` | **Impostos:** `R$ {res.get('valor_impostos', 0):.2f}`")
                    st.subheader("Escopo Final"); [st.write(f"- {item['descricao']}") for item in orc_data.get('escopo_final', [])]
                    st.subheader("Dados do Briefing"); briefing_dict = orc_data.get('briefing', {}); [st.markdown(f"**{k.replace('_', ' ').title()}:** {v}") for k, v in briefing_dict.items()]
